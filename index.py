...
async def get_history(symbol: str, exchange: str, timeframe: str):
    ...
    if any(op in clean_sym for op in ['+', '-', '*', '/']):
        ...
        for t in tokens:
            combined[f"{t}_o"] = ticker_dfs[t].loc[common_idx, 'Open']
            combined[f"{t}_h"] = ticker_dfs[t].loc[common_idx, 'High']
            combined[f"{t}_l"] = ticker_dfs[t].loc[common_idx, 'Low']
            combined[f"{t}_c"] = ticker_dfs[t].loc[common_idx, 'Close']
            
        def repl(m):
            tk = m.group(0)
            return tk if tk.isdigit() else f"__SYM__{tk}"
        tok_str = re.sub(r'[A-Za-z0-9_\^]+', repl, clean_sym)
        
        ex_o, ex_h, ex_l, ex_c = [tok_str.replace("__SYM__", "")] * 4
        for t in tokens:
            ex_o = re.sub(rf'\b{t}\b', f"{t}_o", ex_o)
            ex_h = re.sub(rf'\b{t}\b', f"{t}_h", ex_h)
            ex_l = re.sub(rf'\b{t}\b', f"{t}_l", ex_l)
            ex_c = re.sub(rf'\b{t}\b', f"{t}_c", ex_c)
            
        res_o, res_h, res_l, res_c = combined.eval(ex_o), combined.eval(ex_h), combined.eval(ex_l), combined.eval(ex_c)
        candles = []
        for idx in common_idx:
            o, h, l, c = res_o.loc[idx], res_h.loc[idx], res_l.loc[idx], res_c.loc[idx]
            candles.append({"time": int(idx.timestamp()), "open": round(float(o), 2), "high": round(float(max(h, o, c)), 2), "low": round(float(min(l, o, c)), 2), "close": round(float(c), 2)})
        return {"symbol": clean_sym, "candles": candles}
    ...
    candles = [{"time": int(idx.timestamp()), "open": round(float(row["Open"]), 2), "high": round(float(row["High"]), 2), "low": round(float(row["Low"]), 2), "close": round(float(row["Close"]), 2)} for idx, row in df.iterrows()]
    return {"symbol": clean_sym, "candles": candles}
    ...
    for sym in sym_list:
        ...
        if any(op in clean for op in ['+', '-', '*', '/']):
            ...
            if all(t in t_quotes for t in tokens):
                ep, eprv = clean, clean
                for t in tokens:
                    ep = re.sub(rf'\b{t}\b', str(t_quotes[t]['price']), ep)
                    eprv = re.sub(rf'\b{t}\b', str(t_quotes[t]['price'] - t_quotes[t]['change']), eprv)
                try:
                    cp, pp = float(pd.eval(ep)), float(pd.eval(eprv))
