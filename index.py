...
async def fetch_df(ticker: str, period: str, interval: str):
    ...
    df = await loop.run_in_executor(None, get_data)
    if not df.empty:
...
async def get_history(symbol: str, exchange: str, timeframe: str):
    ...
    try:
        ...
        if any(op in clean_sym for op in ['+', '-', '*', '/']):
            ...
            ticker_dfs = {t: dfs[idx] for idx, t in enumerate(tokens) if not dfs[idx].empty}
            if len(ticker_dfs) != len(tokens): raise HTTPException(status_code=404, detail="Data missing for basket symbols")
...
async def get_quotes(symbols: str, exchange: str):
    ...
    t_quotes = {}
    for t in all_tks:
        def do_fetch(tk=t): return fetch_q(tk)
        r = await loop.run_in_executor(None, do_fetch)
        if r: t_quotes[t] = r
